module.exports = {
  ...
  async execute(message, args) {
    ...
    if (option === 'add') {
      ...
      if (await message.client.tags.Tags.findOne({ where: { name: tagName, guild_id: message.guild.id } }))
        return message.reply('이미 존재하는 태그입니다.');
      
      try {
        // equivalent to: INSERT INTO tags (name, description, username) values (?, ?, ?);
        const tag = await message.client.tags.Tags.create({
          name: tagName,
      ...
      const affectedRows = await message.client.tags.Tags.findOne({ where: { name: tagName, guild_id: message.guild.id } });
      ...
      const tag = await message.client.tags.Tags.findOne({ where: { name: tagName, guild_id: message.guild.id } });
      if (tag) {
      ...
      const rowCount = await message.client.tags.Tags.findOne({ where: { name: tagName, guild_id: message.guild.id } });
      ...
      const tagList = await message.client.tags.Tags.findAll({ attributes: ['name'], where: { guild_id: message.guild.id } });
      ...
      const tagList = await message.client.tags.Tags.findAll({ limit: 10, where: { guild_id: message.guild.id }, order: [['usage_count', 'DESC']] });
      ...
      message.client.tags.Tags.sync({ force: true });
